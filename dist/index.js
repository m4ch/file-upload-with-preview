...
class G {
  constructor(A, g = {}) {
    ...
    const { browse: e, chooseFile: n, label: l, selectedCount: r } = g.text || {};
    this.options.text.chooseFile = n ?? this.options.text.chooseFile, this.options.text.browse = e ?? this.options.text.browse, this.options.text.label = l ?? u, this.options.text.selectedCount = r ?? this.options.text.selectedCount;
    const t = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`);
    if (!t)
      throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    this.el = t, this.el.innerHTML += `
    ...
    `;
    const s = this.el.querySelector(".custom-file-container .input-hidden"), o = this.el.querySelector(".custom-file-container .input-visible"), c = this.el.querySelector(".custom-file-container .image-preview"), d = this.el.querySelector(".custom-file-container .clear-button");
    if (s != null && o != null && c != null && d != null)
      this.inputHidden = s, this.inputVisible = o, this.inputVisible.innerHTML = this.options.text.chooseFile, this.imagePreview = c, this.clearButton = d;
    else
      throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const { backgroundImage: h, baseImage: S, successFileAltImage: J, successPdfImage: R, successVideoImage: p } = g.images || {};
    this.options.images.baseImage = S ?? this.options.images.baseImage, this.options.images.successPdfImage = R ?? this.options.images.successPdfImage, this.options.images.successVideoImage = p ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = J ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = h ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.addBrowseButton(this.options.text.browse), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      ...
      if (g) {
        ...
        if (g.matches(".custom-file-container .image-preview-item")) {
          const C = g.querySelector(".image-preview-item-clear-icon"), B = C == null ? void 0 : C.getAttribute("data-upload-name"), E = this.cachedFileArray.findIndex(({ name: e }) => e === B);
          if (E < 0)
            return;
          const I = {
            detail: {
    ...
    A.forEach(
      (B, E) => C.push(
        new Promise((I) => {
          try {
            const Q = "image/jpeg";
            fetch(B, { mode: "cors" }).then((e) => e.blob()).then((e) => {
              var t, s, o;
              const n = (o = (s = (t = B.split("#")[0]) == null ? void 0 : t.split("?")[0]) == null ? void 0 : s.split("/")) == null ? void 0 : o.pop();
              let l = "preset-file";
              n !== void 0 && (l = n);
              const r = new File([e], l, {
                type: e.type || Q
              });
              console.log(r), g.splice(E, 0, r), I();
            });
          } catch (Q) {
      ...
      console.log("resolveAll", g), Object.values(g).forEach((B) => {
        this.addFiles([B]);
      });
    });
  }
  ...
  refreshPreviewPanel() {
    ...
    Array.from(g).forEach((B) => B.classList.add(y)), setTimeout(() => {
...
export {
  ...
  M as DEFAULT_BROWSE_TEXT,
  D as DEFAULT_CHOOSE_FILE_TEXT,
  P as DEFAULT_FILES_SELECTED_TEXT,
  u as DEFAULT_LABEL_TEXT,
  F as DEFAULT_SUCCESS_FILE_ALT_IMAGE,
  v as DEFAULT_SUCCESS_PDF_IMAGE,
  k as DEFAULT_SUCCESS_VIDEO_IMAGE,
  a as Events,
  G as FileUploadWithPreview,
  y as MULTI_ITEM_CLEAR_ANIMATION_CLASS,
