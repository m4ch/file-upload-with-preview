var o = /* @__PURE__ */ ((t) => (t.IMAGE_ADDED = "fileUploadWithPreview:imagesAdded", t.IMAGE_DELETED = "fileUploadWithPreview:imageDeleted", t.CLEAR_BUTTON_CLICKED = "fileUploadWithPreview:clearButtonClicked", t.IMAGE_MULTI_ITEM_CLICKED = "fileUploadWithPreview:imageMultiItemClicked", t))(o || {});
const m = ":upload:", f = "data:image/png;base64,", k = "data:image/png;base64,", y = "data:image/png;base64,", M = "data:image/png;base64,", D = "data:image/png;base64,", P = "multi-item-clear-animation", L = "Choose file...", U = "Browse", G = "files selected", h = "Upload", b = () => Math.random().toString(16).slice(2), q = (t) => {
  var A, g, B;
  try {
    const e = (B = (g = (A = t.split("#")[0]) == null ? void 0 : A.split("?")[0]) == null ? void 0 : g.split("/")) == null ? void 0 : B.pop();
    let E = "preset-file";
    return e !== void 0 && (E = e), E;
  } catch (e) {
    return console.warn("Cannot parse filename: ", e), "preset-file";
  }
};
class T {
  constructor(A, g = {}) {
    if (this.options = {
      accept: "*",
      defaultSelectedFileIndex: null,
      images: {
        backgroundImage: D,
        baseImage: f,
        successFileAltImage: M,
        successPdfImage: k,
        successVideoImage: y
      },
      maxFileCount: 0,
      multiple: !1,
      presetFiles: [],
      showDeleteButtonOnImages: !0,
      showFavoriteButton: !1,
      showMoveImageButtons: !0,
      text: {
        browse: U,
        chooseFile: L,
        label: h,
        selectedCount: G
      }
    }, !A)
      throw new Error(
        "No uploadId found. You must initialize file-upload-with-preview with a unique uploadId."
      );
    this.uploadId = A, this.cachedFileArray = [];
    const {
      accept: B,
      maxFileCount: e,
      multiple: E,
      presetFiles: C,
      showDeleteButtonOnImages: Q,
      showMoveImageButtons: i,
      showFavoriteButton: s,
      defaultSelectedFileIndex: I
    } = g;
    this.options.showDeleteButtonOnImages = Q ?? !0, this.options.maxFileCount = e ?? 0, this.options.presetFiles = C ?? [], this.options.multiple = E ?? !1, this.options.accept = B ?? this.options.accept, this.options.showMoveImageButtons = i ?? this.options.showMoveImageButtons, this.options.showFavoriteButton = s ?? this.options.showFavoriteButton, this.selectedFileIndex = I ?? this.options.defaultSelectedFileIndex;
    const { browse: d, chooseFile: u, label: S, selectedCount: p } = g.text || {};
    this.options.text.chooseFile = u ?? this.options.text.chooseFile, this.options.text.browse = d ?? this.options.text.browse, this.options.text.label = S ?? h, this.options.text.selectedCount = p ?? this.options.text.selectedCount;
    const a = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`);
    if (!a)
      throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    this.el = a, this.el.innerHTML += `
      <div class="label-container">
        <label>${this.options.text.label}</label>
        <a class="clear-button" href="javascript:void(0)" title="Clear Image">
          &times;
        </a>
      </div>
      <label class="input-container">
        <input
          accept="${this.options.accept}"
          aria-label="Choose File"
          class="input-hidden"
          id="file-upload-with-preview-${A}"
          ${this.options.multiple ? "multiple" : ""}
          type="file"
        />
        <span class="input-visible"></span>
      </label>
      <div class="image-preview"></div>
    `;
    const n = this.el.querySelector(".custom-file-container .input-hidden"), r = this.el.querySelector(".custom-file-container .input-visible"), l = this.el.querySelector(".custom-file-container .image-preview"), c = this.el.querySelector(".custom-file-container .clear-button");
    if (n != null && r != null && l != null && c != null)
      this.inputHidden = n, this.inputVisible = r, this.inputVisible.innerHTML = this.options.text.chooseFile, this.imagePreview = l, this.clearButton = c;
    else
      throw new Error(`Cannot find all necessary elements for the id: ${this.uploadId}`);
    const { backgroundImage: J, baseImage: v, successFileAltImage: w, successPdfImage: F, successVideoImage: R } = g.images || {};
    this.options.images.baseImage = v ?? this.options.images.baseImage, this.options.images.successPdfImage = F ?? this.options.images.successPdfImage, this.options.images.successVideoImage = R ?? this.options.images.successVideoImage, this.options.images.successFileAltImage = w ?? this.options.images.successFileAltImage, this.options.images.backgroundImage = J ?? this.options.images.backgroundImage, this.addImagesFromPath(this.options.presetFiles), this.addBrowseButton(this.options.text.browse), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.bindClickEvents(), this.refreshPreviewPanel();
  }
  bindClickEvents() {
    this.inputHidden.addEventListener(
      "change",
      (A) => {
        const g = A.target, { files: B } = g;
        B != null && (this.addFiles(B), this.refreshPreviewPanel(), g.value = "");
      },
      !0
    ), this.clearButton.addEventListener(
      "click",
      () => {
        const A = {
          detail: {
            uploadId: this.uploadId
          }
        }, g = new CustomEvent(o.CLEAR_BUTTON_CLICKED, A);
        window.dispatchEvent(g), this.resetPreviewPanel();
      },
      !0
    ), this.imagePreview.addEventListener("click", (A) => {
      var E;
      const g = A.target;
      if (!g)
        return;
      const B = g.getAttribute("data-upload-name") ?? ((E = g.querySelector(".image-preview-item-clear-icon")) == null ? void 0 : E.getAttribute("data-upload-name")), e = this.cachedFileArray.findIndex(({ name: C }) => C === B);
      if (g.matches(".custom-file-container .image-preview-item-clear-icon") && this.deleteFileAtIndex(e), g.matches(".custom-file-container .image-preview-item-favorite-icon") && this.selectFileAtIndex(e), g.matches(".custom-file-container .image-preview-item-move-left-icon") && this.moveFileTo(e, -1), g.matches(".custom-file-container .image-preview-item-move-right-icon") && this.moveFileTo(e, 1), g.matches(".custom-file-container .image-preview-item")) {
        if (e < 0)
          return;
        const C = {
          detail: {
            cachedFileArray: this.cachedFileArray,
            file: this.cachedFileArray[e],
            index: e,
            uploadId: this.uploadId
          }
        }, Q = new CustomEvent(o.IMAGE_MULTI_ITEM_CLICKED, C);
        window.dispatchEvent(Q);
      }
    });
  }
  selectFileAtIndex(A) {
    var B;
    if (A < 0 || !this.cachedFileArray[A])
      return;
    const g = document.querySelector(
      `[data-upload-name="${this.cachedFileArray[A].name}"]`
    );
    A === this.selectedFileIndex ? (g == null || g.classList.remove("image-preview-item-selected"), this.selectedFileIndex = null) : (this.selectedFileIndex = A, (B = document.querySelector(".image-preview-item-selected")) == null || B.classList.remove("image-preview-item-selected"), g == null || g.classList.add("image-preview-item-selected"));
  }
  moveFileTo(A, g) {
    if (A + g < 0 || A + g > this.cachedFileArray.length - 1)
      return;
    const B = this.cachedFileArray[A];
    this.cachedFileArray[A] = this.cachedFileArray[A + g], this.cachedFileArray[A + g] = B, this.options.showFavoriteButton && (this.selectedFileIndex = this.selectedFileIndex + g), this.refreshPreviewPanel();
  }
  addImagesFromPath(A) {
    const g = [], B = [];
    A.forEach(
      (e, E) => B.push(
        new Promise((C) => {
          try {
            const Q = "image/jpeg";
            fetch(e, { mode: "cors" }).then((i) => i.blob()).then((i) => {
              const s = q(e), I = new File([i], s, {
                type: i.type || Q
              });
              g.splice(E, 0, I), C();
            });
          } catch (Q) {
            Q instanceof Error && console.warn(`${Q.message.toString()}`), console.warn("Image cannot be added to the cachedFileArray.");
          }
        })
      )
    ), Promise.all(B).then(() => {
      Object.values(g).forEach((e) => {
        this.addFiles([e]);
      });
    });
  }
  addFiles(A) {
    if (!A.length)
      return;
    let g = Array.from(A);
    if (this.options.multiple && this.options.maxFileCount > 0) {
      const C = this.cachedFileArray.length + g.length - this.options.maxFileCount;
      C > 0 && (g = g.slice(0, g.length - C));
    }
    this.options.multiple || (this.cachedFileArray = []), g.forEach((E) => {
      const C = new File(
        [E],
        `${E.name || "fallback-name"}${m}${b()}`,
        {
          type: E.type
        }
      );
      this.cachedFileArray.push(C);
    });
    const B = {
      detail: {
        addedFilesCount: g.length,
        cachedFileArray: this.cachedFileArray,
        files: A,
        uploadId: this.uploadId
      }
    }, e = new CustomEvent(o.IMAGE_ADDED, B);
    window.dispatchEvent(e);
  }
  addFileToPreviewPanel(A, g) {
    return new Promise((B) => {
      this.cachedFileArray.length === 0 ? this.inputVisible.innerHTML = this.options.text.chooseFile : this.cachedFileArray.length === 1 ? this.inputVisible.textContent = A.name.split(m)[0] : this.inputVisible.innerHTML = `${this.cachedFileArray.length} ${this.options.text.selectedCount}`;
      const e = new FileReader();
      e.readAsDataURL(A), e.onload = () => {
        if (!this.options.multiple) {
          let I = this.options.images.successFileAltImage;
          A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? I = `url("${e.result}")` : A.type.match("application/pdf") ? I = `url("${this.options.images.successPdfImage}")` : A.type.match("video/*") && (I = `url("${this.options.images.successVideoImage}")`), this.imagePreview.style.backgroundImage = I;
          return;
        }
        this.imagePreview.style.backgroundImage = `url("${this.options.images.backgroundImage}")`;
        const E = (I) => `
          <span class="image-preview-item-clear">
            <span class="image-preview-item-clear-icon" data-upload-name="${I}">
              &times;
            </span>
          </span>
        `, C = (I) => `
          <span class="image-preview-item-favorite">
            <span class="image-preview-item-favorite-icon" data-upload-name="${I}">
              ★
            </span>
          </span>
        `, Q = (I) => `
          <span class="image-preview-item-move-left">
            <span class="image-preview-item-move-left-icon" data-upload-name="${I}">
              &lt;
            </span>
          </span>
        `, i = (I) => `
          <span class="image-preview-item-move-right">
            <span class="image-preview-item-move-right-icon" data-upload-name="${I}">
              &gt;
            </span>
          </span>
        `;
        let s = this.options.images.successFileAltImage;
        A.type.match("image/png") || A.type.match("image/jpeg") || A.type.match("image/webp") || A.type.match("image/gif") ? s = e.result : A.type.match("application/pdf") ? s = this.options.images.successPdfImage : A.type.match("video/*") && (s = this.options.images.successVideoImage), B(`
          <div
            class="image-preview-item ${g === this.selectedFileIndex ? "image-preview-item-selected" : ""}"
            data-upload-name="${A.name}"
            style="background-image: url('${s}'); "
          >
            ${this.options.showDeleteButtonOnImages ? E(A.name) : ""}
            ${this.options.showFavoriteButton && this.options.multiple ? C(A.name) : ""}
            ${this.options.showMoveImageButtons && this.options.multiple ? Q(A.name) : ""}
            ${this.options.showMoveImageButtons && this.options.multiple ? i(A.name) : ""}
          </div>`);
      };
    });
  }
  replaceFiles(A) {
    if (!A.length)
      throw new Error("Array must contain at least one file.");
    this.cachedFileArray = A, this.refreshPreviewPanel();
  }
  replaceFileAtIndex(A, g) {
    if (!this.cachedFileArray[g])
      throw new Error(`There is no file at index: ${g}`);
    this.cachedFileArray[g] = A, this.refreshPreviewPanel();
  }
  deleteFileAtIndex(A) {
    if (!this.cachedFileArray[A])
      throw new Error(`There is no file at index ${A}`);
    this.cachedFileArray = [
      ...this.cachedFileArray.slice(0, A),
      ...this.cachedFileArray.slice(A + 1)
    ], this.refreshPreviewPanel();
    const g = {
      detail: {
        cachedFileArray: this.cachedFileArray,
        currentFileCount: this.cachedFileArray.length,
        index: A,
        uploadId: this.uploadId
      }
    }, B = new CustomEvent(o.IMAGE_DELETED, g);
    window.dispatchEvent(B);
  }
  refreshPreviewPanel() {
    const g = this.imagePreview.querySelectorAll(".image-preview-item");
    Array.from(g).forEach((e) => e.classList.add(P)), setTimeout(() => {
      if (this.imagePreview.innerHTML = "Loading...", !this.cachedFileArray.length) {
        this.resetPreviewPanel();
        return;
      }
      const e = [];
      this.cachedFileArray.forEach(
        (E, C) => e.push(this.addFileToPreviewPanel(E, C))
      ), Promise.all(e).then((E) => {
        this.imagePreview.innerHTML = "", E.forEach((C) => {
          this.imagePreview.innerHTML += C;
        });
      });
    }, 200);
  }
  addBrowseButton(A) {
    this.inputVisible.innerHTML += `<span class="browse-button">${A}</span>`;
  }
  emulateInputSelection() {
    this.inputHidden.click();
  }
  resetPreviewPanel() {
    this.inputHidden.value = "", this.inputVisible.innerHTML = this.options.text.chooseFile, this.addBrowseButton(this.options.text.browse), this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`, this.imagePreview.innerHTML = "", this.cachedFileArray = [];
  }
}
export {
  D as DEFAULT_BACKGROUND_IMAGE,
  f as DEFAULT_BASE_IMAGE,
  U as DEFAULT_BROWSE_TEXT,
  L as DEFAULT_CHOOSE_FILE_TEXT,
  G as DEFAULT_FILES_SELECTED_TEXT,
  h as DEFAULT_LABEL_TEXT,
  M as DEFAULT_SUCCESS_FILE_ALT_IMAGE,
  k as DEFAULT_SUCCESS_PDF_IMAGE,
  y as DEFAULT_SUCCESS_VIDEO_IMAGE,
  o as Events,
  T as FileUploadWithPreview,
  P as MULTI_ITEM_CLEAR_ANIMATION_CLASS,
  m as UNIQUE_ID_IDENTIFIER
};
